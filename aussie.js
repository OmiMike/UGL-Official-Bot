...
module.exports = async (client) => {
    client.db.query("SELECT * FROM guild_settings WHERE guild_ID= '899801075786342480';", async function (err, streamRows) {
        ...
        if(channelIsLive == false) {
            if(streamRows[0].isAussieLive == 1) {
                client.db.query("UPDATE guild_settings SET `isAussieLive` = '0' WHERE guild_ID= '899801075786342480';");
                console.log("Notifications turned off")
            }
        } else {
            //<@&${config.otherPing}> 
            if(streamRows[0].isAussieLive == 0) {
                ...
                client.db.query("UPDATE guild_settings SET `isAussieLive` = '1' WHERE guild_ID= '899801075786342480';");
            }
        }
    });
}