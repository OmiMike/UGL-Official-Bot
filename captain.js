...
module.exports = async (client) => {
    client.db.query("SELECT * FROM guild_settings WHERE guild_ID= '899801075786342480';", async function (err, streamRows) {
        const streamers = "captaincaldwellvr";
        ...
        if(channelIsLive == false) {
            if(streamRows[0].isFaceLive == 1) {
                client.db.query("UPDATE guild_settings SET `isFaceLive` = '0' WHERE guild_ID= '899801075786342480';");
                console.log("Notifications turned off")
            }
        } else {
            //<@&${config.otherPing}> 
            if(streamRows[0].isFaceLive == 0) {
                ...
                client.db.query("UPDATE guild_settings SET `isFaceLive` = '1' WHERE guild_ID= '899801075786342480';");
            }
        }
    });
}